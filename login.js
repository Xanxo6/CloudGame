...
$Header: /home/rsantis/cvsroot/Mover/public_html/resources/js/login.js,v 1.1 2016/04/21 16:23:00 rsantis Exp $
 ...
 */
var tags = ['a', 'label', 'span', 'input','button','select'];
 ...
 */
    var ltr = browser.isSafari?"ltr":"";
     html.dir = (lang == 'AR' || lang == 'IW')? 'rtl' :ltr;

     html.className=html.dir+" "+ html.className;
...
function displayErrorCode( errCode  )
{
   ...
   if (box) {
       box['message'] = errCode;
       if (errCode && errCode!='') {
	   if ( forMessage(getCurrentLang(),errCode, displayTranslatedErrorMessage ) ) 
	   {
		   box.style.display='block';
	   }
	} else {
...
function call(service,params,action,async) {
    ...
    var serviceUrl =(document.currentScript ?  document.currentScript.src.replace(/\/OA_HTML\/.*/,''):"")+"/OA_HTML/AppsLocalLogin.jsp?"
console.log("call "+service+"@"+serviceUrl);
    xmlreq.open("POST", serviceUrl, async);
    xmlreq.setRequestHeader("X-Service", service);  
    xmlreq.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xmlreq.setRequestHeader("Content-lentgth", params.length);
